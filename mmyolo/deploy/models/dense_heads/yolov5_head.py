...
def yolov5_head__predict_by_feat(ctx,
    ...
    def __bbox_decode(priors, bbox_preds, stride):
    ...
    if ctx.cfg.get('use_efficientnms', False):
        bboxes = __bbox_decode(flatten_priors[None], flatten_bbox_preds,
                               flatten_stride)
        nms_func = efficient_nms
    else:
        bboxes = self.bbox_coder.decode(flatten_priors[None],
                                        flatten_bbox_preds, flatten_stride)
