...
@FUNCTION_REWRITER.register_rewriter(
    func_name='mmyolo.models.dense_heads.yolox_head.'
...
def yolox_head__predict_by_feat(ctx,
    ...
    def __bbox_decode(priors, bbox_preds):
    ...
    cls_scores = torch.cat(flatten_cls_preds, dim=1).sigmoid()
    flatten_bbox_preds = torch.cat(flatten_bbox_preds, dim=1)
    flatten_objectness = torch.cat(flatten_objectness, dim=1).sigmoid()

    if ctx.cfg.get('use_efficientnms', False):
        bboxes = __bbox_decode(flatten_priors, flatten_bbox_preds)
        nms_func = efficient_nms
    else:
        bboxes = self.bbox_coder.decode(flatten_priors[..., :2],
                                        flatten_bbox_preds, flatten_priors[...,
                                                                           2])
